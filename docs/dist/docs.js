...
const docs = {
	...
	description: "Disclosure-style menu button behavior for native links, buttons, and form controls, with keyboard navigation and optional add-ons.",
  ...
  createMenuButton(root);
	...
	keyboard: [
		...
		},
		{
			...
			description: "Uses native checkbox activation and leaves the panel open; Enter keeps its native non-activation behavior."
		},
		{
		...
		},
		{
			...
			description: "Moves focus to the next item whose data, ARIA, native form, or text label starts with the query; editable controls retain typing."
		}
	],
	api: [
		{
		...
		},
		{
			...
			description: "Adds local filtering and optional delayed, deduplicated polite result-count announcements."
		}
	],
	events: [
		{
	...
	examples: [
		...
		},
		{
			...
			description: "Native checkbox controls that keep the disclosure open, including a disabled option.",
			path: "index.html#example-checkboxes"
		},
		{
			name: "Add-on cleanup ownership",
			description: "Reverse-order add-on cleanup followed by core teardown and safe reinitialization.",
			path: "index.html#example-cleanup"
		},
		{
			name: "Async retry and stale responses",
			description: "Application-owned cancellation, request ordering, retry UI, rendering, and async-state updates.",
			path: "index.html#example-async"
		},
		{
			name: "Filter result-count announcements",
			description: "Opt-in localized result counts with delayed updates, message deduplication, and an independent empty state.",
