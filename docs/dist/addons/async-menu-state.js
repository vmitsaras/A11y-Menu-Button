...
//#region src/addons/async-menu-state.ts
...
const VALID_STATES = /* @__PURE__ */ new Set([
	"loading",
	"empty",
	"error",
	"ready"
...
function getMessage(root, options, state) {
	if (options.message?.trim()) return options.message.trim();
	if (state === "loading") return root.dataset.menuLoadingMessage || DEFAULT_LOADING_MESSAGE;
	if (state === "error") return root.dataset.menuErrorMessage || DEFAULT_ERROR_MESSAGE;
...
function updateStateElement(stateElement, root, options, state) {
	const message = stateElement.querySelector(`.${STATE_MESSAGE_CLASS}`) ?? stateElement;
...
function createStateElement(root, options, state) {
	const element = document.createElement("div");
	const message = document.createElement("p");
	const count = parseSafeInteger(options.skeletonCount ?? root.dataset.menuLoadingSkeletonCount, DEFAULT_SKELETON_COUNT, 1);
...
function setItemsHidden(panel, hidden, originalHidden) {
	if (!hidden) {
...
function enhanceAsyncMenuState(root, options = {}) {
	if (!(root instanceof HTMLElement)) throw new TypeError("enhanceAsyncMenuState: root must be an HTMLElement");
	...
	const initialState = options.state ?? (root.dataset.menuLoading === "true" ? "loading" : "ready");
	const existingStateElement = panel.querySelector(`:scope > .${STATE_CLASS}`);
	const stateElement = options.stateElement ?? (existingStateElement instanceof HTMLElement ? existingStateElement : null) ?? createStateElement(root, options, initialState);
	...
	const stateMessage = stateElement.querySelector(`.${STATE_MESSAGE_CLASS}`) ?? stateElement;
	const initialStateMessageChildren = generatedStateElement ? [] : Array.from(stateMessage.childNodes);
	const initialStateElementAttributes = generatedStateElement ? null : {
	...
	let controller;
	const applyState = (nextState, nextOptions, reason) => {
		if (destroyed) return;
		...
		setItemsHidden(panel, messageState && merged.hideItems !== false, originalHidden);
	...
	controller = {
		...
		destroy() {
			...
			if (generatedStateElement) stateElement.remove();
				...
				restoreAttribute("data-menu-async-state", initialStateElementAttributes.state);
				stateElement.hidden = initialStateElementAttributes.hidden;
				stateMessage.replaceChildren(...initialStateMessageChildren);
				if (!initialStateElementParent) stateElement.remove();
				else if (stateElement.parentNode !== initialStateElementParent) initialStateElementParent.insertBefore(stateElement, initialStateElementNextSibling);
			}
...
function setAsyncMenuLoading(root, loading = true, options = {}) {
	const controller = controllers.get(root) ?? enhanceAsyncMenuState(root, options);
	controller.setLoading(loading, options);
	return controller;
}
function enhanceAsyncMenuStates(root = document, options = {}) {
	return getMatchingRoots(root, ROOT_SELECTOR).map((menu) => enhanceAsyncMenuState(menu, options));
}
//#endregion
export { enhanceAsyncMenuState as default, enhanceAsyncMenuState, enhanceAsyncMenuStates, setAsyncMenuLoading };

//# sourceMappingURL=async-menu-state.js.map