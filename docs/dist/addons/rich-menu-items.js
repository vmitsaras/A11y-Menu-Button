import { n as PANEL_SELECTOR, r as getMatchingRoots } from "../shared-CfHq7gqB.js";
//#region src/addons/rich-menu-items.ts
...
function enhanceRichMenuItems(root = document, options = {}) {
	...
	const enhance = (item) => {
		...
		if (item.dataset.menuIcon) item.append(decorativeSpan(ICON_CLASS, item.dataset.menuIcon));
		item.append(content);
		if (item.dataset.menuDescription) content.append(decorativeSpan(DESCRIPTION_CLASS, item.dataset.menuDescription));
		if (item.dataset.menuShortcut) item.append(decorativeSpan(SHORTCUT_CLASS, item.dataset.menuShortcut));
		if (originalLabel && !item.hasAttribute("aria-label")) item.setAttribute("aria-label", originalLabel);
	};
	const refresh = () => {
		if (destroyed) return;
...
//#endregion
export { enhanceRichMenuItems as default, enhanceRichMenuItems };

//# sourceMappingURL=rich-menu-items.js.map